...
class ServiceMonitoring(models.Model):
    ...
    health_check_url = models.URLField(help_text="URL pour vérifier la santé du service")
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='unknown')
    last_check = models.DateTimeField(null=True, blank=True)
    response_time_ms = models.IntegerField(null=True, blank=True, help_text="Temps de réponse en millisecondes")
    error_message = models.TextField(blank=True, help_text="Dernier message d'erreur")
    uptime_percentage = models.FloatField(default=0.0, help_text="Pourcentage de disponibilité sur 24h")
    is_critical = models.BooleanField(default=True, help_text="Service critique pour le système")
...
class ServiceHealthHistory(models.Model):
    ...
    service = models.ForeignKey(ServiceMonitoring, on_delete=models.CASCADE, related_name='health_history')
...
class SystemAlert(models.Model):
    ...
    service = models.ForeignKey(ServiceMonitoring, on_delete=models.CASCADE, null=True, blank=True)
