...
class CourseAdminForm(forms.ModelForm):
    ...
    paragraphs_text = forms.CharField(
        widget=forms.Textarea(
            attrs={"rows": 6, "placeholder": "Un paragraphe par ligne"}
        ),
        required=False,
    ...
    class Meta:
        ...
        widgets = {
            ...
            "duration_text": forms.TextInput(
                attrs={"placeholder": "ex: 4 à 6 semaines, 20 heures"}
            ),
        }
        ...
        if self.instance.pk:
            ...
            if self.instance.examples:
                self.fields["examples_text"].initial = "\n---\n".join(
                    self.instance.examples
                )

            if self.instance.json_categories:
                self.fields["categories_text"].initial = ", ".join(
                    self.instance.json_categories
                )
    ...
    def save(self, commit=True):
        ...
        if self.cleaned_data.get("paragraphs_text"):
            paragraphs = [
                p.strip()
                for p in self.cleaned_data["paragraphs_text"].split("\n")
                if p.strip()
            ]
        ...
        if self.cleaned_data.get("content_lists_text"):
            ...
            for list_text in self.cleaned_data["content_lists_text"].split("---"):
                if list_text.strip():
                    items = [
                        item.strip()
                        for item in list_text.strip().split("\n")
                        if item.strip()
                    ]
                    if items:
                        lists.append(items)
        ...
        if self.cleaned_data.get("examples_text"):
            examples = [
                ex.strip()
                for ex in self.cleaned_data["examples_text"].split("---")
                if ex.strip()
            ]
        ...
        if self.cleaned_data.get("categories_text"):
            categories = [
                cat.strip()
                for cat in self.cleaned_data["categories_text"].split(",")
                if cat.strip()
            ]
...
class CourseAdmin(admin.ModelAdmin):
    ...
    def export_to_elasticsearch_format(self, request, queryset):
        ...
        response["Content-Disposition"] = (
            'attachment; filename="courses_elasticsearch.json"'
        )
        return response

    export_to_elasticsearch_format.short_description = (
        "Exporter au format Elasticsearch"
    )
