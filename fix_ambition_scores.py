...
def generate_varied_goals():
    ...
    short_term_goals = [
        "Apprendre Python", "Maîtriser Docker", "Stage en entreprise", 
        "Certification AWS", "Projet personnel", "Formation DevOps",
        "Apprendre React", "Certification Azure", "Bootcamp intensif",
        "Formation Data Science", "Apprendre Kubernetes", "Certification GCP",
        "Projet open source", "Formation cybersécurité", "Apprendre Node.js",
        "Certification Scrum", "Formation IA", "Apprendre Vue.js",
        "Stage développement", "Formation cloud", "Apprendre Angular",
        "Certification PMP", "Projet freelance", "Formation blockchain",
        "Apprendre Flutter", "Certification ITIL", "Formation mobile",
        "Apprendre Rust", "Certification CISSP", "Formation web3"
    ]
    
    # Objectifs long terme variés
    long_term_goals = [
        "Architecte Cloud", "Créer une startup", "Spécialisation Big Data",
        "Freelance", "Lead Developer", "Data Scientist",
        "Reconvertion en développement", "CTO", "Expert IA",
        "Consultant indépendant", "Product Manager", "DevOps Engineer",
        "Spécialisation cybersécurité", "Créer une entreprise", "Tech Lead",
        "Expert Blockchain", "Manager technique", "Spécialisation mobile",
        "Créer une agence", "Expert Cloud", "Spécialisation web3",
        "Directeur technique", "Expert Data", "Spécialisation DevOps",
        "Créer une plateforme", "Expert Sécurité", "Spécialisation IA",
        "Consultant senior", "Expert Mobile", "Spécialisation Cloud"
    ]
...
def create_varied_ambition_scores(df):
    ...
    ambition_factors = {
        'age': lambda x: 1.2 if x < 25 else 1.0 if x < 35 else 0.8,  # Plus ambitieux quand jeune
        'total_experience_years': lambda x: 1.1 if x < 5 else 1.0 if x < 15 else 0.9,  # Moins d'exp = plus ambitieux
        'highest_academic_level': {
            'Baccalauréat': 1.3, 'Bac+2': 1.2, 'Bac+3': 1.1, 
            'Bac+5': 1.0, 'Bac+8': 0.9
        }
    }
    ...
    for idx, row in df.iterrows():
        ...
        exp_factor = ambition_factors['total_experience_years'](row['total_experience_years'])
...
def main():
    ...
    current_ambition_scores = [s + l for s, l in zip(current_short_counts, current_long_counts)]
    
    print(f"   • Score d'ambition actuel: {np.mean(current_ambition_scores):.1f} ± {np.std(current_ambition_scores):.1f}")
    print(f"   • Objectifs court terme: {np.mean(current_short_counts):.1f} ± {np.std(current_short_counts):.1f}")
    print(f"   • Objectifs long terme: {np.mean(current_long_counts):.1f} ± {np.std(current_long_counts):.1f}")
    ...
    print(f"   • Score d'ambition moyen: {np.mean(new_ambition_scores):.1f} ± {np.std(new_ambition_scores):.1f}")
    print(f"   • Objectifs court terme: {np.mean(new_short_counts):.1f} ± {np.std(new_short_counts):.1f}")
    print(f"   • Objectifs long terme: {np.mean(new_long_counts):.1f} ± {np.std(new_long_counts):.1f}")
