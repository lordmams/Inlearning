...
def show_status():
    ...
    for task_name, task_info in status.items():
        ...
        last_run = task_info['last_run'].strftime('%Y-%m-%d %H:%M:%S') if task_info['last_run'] else 'Jamais'
...
def main():
    parser = argparse.ArgumentParser(description='Gestionnaire d\'orchestration InLearning')
    parser.add_argument('command', choices=['start', 'status', 'run', 'list', 'logs'], 
