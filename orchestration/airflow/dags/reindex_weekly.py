...
def create_pre_reindex_snapshot(**context):
    ...
    try:
        ...
        snapshot_url = f"{ELASTICSEARCH_HOST}/_snapshot/{repository_name}/{snapshot_name}"
        response = requests.put(
            snapshot_url,
            headers=get_es_headers(),
        ...
        while wait_time < max_wait_time:
            ...
            if status_response.status_code == 200:
                ...
                if snapshot_state == 'SUCCESS':
                    ...
                    raise Exception(f"Échec de la création du snapshot: {snapshot_name}")
    ...
    try:
        ...
        stats = {
            ...
            'shards': {
                ...
                'primary': len(info_data[ELASTICSEARCH_INDEX]['settings']['index']['number_of_shards'])
            },
            'creation_date': info_data[ELASTICSEARCH_INDEX]['settings']['index']['creation_date'],
            'timestamp': datetime.now().isoformat()
        }
...
def perform_reindexation(**context):
    ...
    try:
        ...
        new_index_name = f"{ELASTICSEARCH_INDEX}-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
...
def validate_reindexation(**context):
    ...
    try:
        ...
        validation_result = {
            ...
            'document_loss_percentage': round(((old_count - new_count) / old_count) * 100, 2) if old_count > 0 else 0,
            'validation_passed': abs(old_count - new_count) <= (old_count * 0.01),  # Tolérance de 1%
            'timestamp': datetime.now().isoformat()
        }
        ...
        if validation_result['validation_passed']:
            ...
            logging.warning(f"⚠️ Validation échouée - Perte de données: {validation_result['document_loss']} documents")
...
def cleanup_old_indices(**context):
    ...
    try:
        ...
        # Liste des indices
        indices_url = f"{ELASTICSEARCH_HOST}/_cat/indices/{ELASTICSEARCH_INDEX}*?format=json"
