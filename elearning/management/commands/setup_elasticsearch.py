...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS("🔍 Configuration d'Elasticsearch..."))
        ...
        try:
            ...
            self.stdout.write(
                f"📊 État du cluster: {health.get('cluster_status', 'unknown')}"
            )
            ...
            if options["delete_index"]:
                ...
                try:
                    elastic_service.client.indices.delete(
                        index=elastic_service.index_name
                    )
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"✅ Index {elastic_service.index_name} supprimé"
                        )
                    )
            ...
            if analytics:
                ...
                self.stdout.write(
                    f'   • Total cours indexés: {analytics.get("total_courses", 0)}'
                )
