...
class CoursePipeline:
    def __init__(
        self, categories_classifier, level_model_path: str, vectorizer_path: str
    ):
        """
    ...
    def process_courses_batch(
        self, courses_data: List[Dict[str, Any]], batch_size: int = 32
    ) -> List[Dict[str, Any]]:
        """
        ...
        for i, course in enumerate(tqdm(courses_data, desc="Traitement des cours")):
            # Classifier la catégorie
            category, category_score = self.categories_classifier.classify_text(
                texts[i]
            )
    ...
    def process_file(
        self, input_path: Path, output_path: Path, batch_size: int = 32
    ) -> None:
        """
        ...
        try:
            ...
            for course in courses:
                if isinstance(course, dict):
                    course_data = (
                        course.get("cours", {}) if "cours" in course else course
                    )
            ...
            logger.info(
                f"Traitement terminé. {len(processed_courses)} cours traités et sauvegardés dans {output_path}"
            )
...
def main():
    ...
    test_course = {
        ...
        "contenus": {
            ...
            "examples": ["x = 10", "for i in range(5):", "    print(i)"],
        },
    }
