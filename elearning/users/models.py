...
class Person(models.Model):
    ...
    gender = models.CharField(
        max_length=1, choices=GENDER_CHOICES, blank=True, verbose_name="Sexe"
    )
    ...
    predicted_level = models.CharField(
        max_length=50, blank=True, null=True, verbose_name="Niveau prédit"
    )
...
class Preferences(models.Model):
    person = models.OneToOneField(
        Person, on_delete=models.CASCADE, related_name="preferences"
    )
    LANGUAGE_CHOICES = [
        ("fr", "Français"),
        ("en", "Anglais"),
    ...
    preferred_language = models.CharField(
        max_length=2, choices=LANGUAGE_CHOICES, verbose_name="Langue préférée"
    )
    ...
    learning_mode = models.CharField(
        ...
        verbose_name="Mode d'apprentissage préféré",
    )
...
class Interest(models.Model):
    preferences = models.ForeignKey(
        Preferences, on_delete=models.CASCADE, related_name="interests"
    )
...
class AcademicBackground(models.Model):
    person = models.OneToOneField(
        Person, on_delete=models.CASCADE, related_name="academic_background"
    )
    ...
    highest_academic_level = models.CharField(
        max_length=10,
        choices=ACADEMIC_LEVEL_CHOICES,
        verbose_name="Niveau académique atteint",
    )
...
class FieldOfStudy(models.Model):
    academic_background = models.ForeignKey(
        AcademicBackground, on_delete=models.CASCADE, related_name="fields_of_study"
    )
...
class ProfessionalBackground(models.Model):
    ...
    total_experience_years = models.IntegerField(
        verbose_name="Années d'expérience totale", null=True, blank=True
    )
...
class Job(models.Model):
    professional_background = models.ForeignKey(
        ProfessionalBackground, on_delete=models.CASCADE, related_name="jobs"
    )
    ...
    description = models.TextField(
        blank=True, verbose_name="Description des responsabilités"
    )
...
class Goals(models.Model):
    person = models.OneToOneField(
        Person, on_delete=models.CASCADE, related_name="goals"
    )
...
class ShortTermGoal(models.Model):
    goals = models.ForeignKey(
        Goals, on_delete=models.CASCADE, related_name="short_term_goals"
    )
    description = models.CharField(
        max_length=255, verbose_name="Objectif à court terme"
    )
...
class LongTermGoal(models.Model):
    goals = models.ForeignKey(
        Goals, on_delete=models.CASCADE, related_name="long_term_goals"
    )
