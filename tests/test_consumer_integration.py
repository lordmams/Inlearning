...
def test_consumer_integration():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"❌ Learning Platform API indisponible (status: {response.status_code})")
            return False
    ...
    test_course = {
        ...
        "cours": {
            ...
            "contenus": {
                "paragraphs": ["Python est un langage de programmation", "Il est facile à apprendre"],
                "lists": [["Variables", "Fonctions", "Classes"], ["Int", "String", "List"]],
    ...
    while time.time() - start_time < max_wait:
        if not test_file_path.exists():
            ...
            if any(f.name.startswith(f"test_course_{timestamp}") for f in processed_folder.glob("*")):
                ...
                if any(f.name.startswith(f"test_course_{timestamp}") for f in error_folder.glob("*")):
    ...
    try:
        response = requests.get(f"{learning_platform_url}/stats/processed-courses", timeout=5)
        if response.status_code == 200:
            stats = response.json()
    ...
    print("\n💡 Architecture finale:")
    print("   Django Interface → Dépôt fichier → Consumer → Learning Platform API → Elasticsearch")
