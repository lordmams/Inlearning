...
class DistributedCourseProcessor:
    ...
    def _init_spark_session(self):
        ...
        try:
            self.spark = SparkSession.builder \
                .appName("InLearning-CourseProcessing") \
            ...
            logger.info(f"✅ Session Spark initialisée avec master: {self.spark_master_url}")
            return True
    ...
    def process_courses_from_json(self, json_path, output_path=None):
        ...
        try:
            ...
            # Charger les données
            df = self.spark.read.option("multiline", "true").json(json_path, schema=course_schema)
    ...
    def _classify_courses_distributed(self, df):
        ...
        try:
            ...
            categories = ["Python", "JavaScript", "Data Science", "Web Development", "Machine Learning", "Autres"]
            ...
            result_df = predictions.withColumn(
                "predicted_category",
                map_category_udf(col("prediction"))
            ).withColumn(
                "category_confidence",
    ...
    def _predict_levels_distributed(self, df):
        ...
        try:
            ...
            level_df = df.withColumn(
                "predicted_level",
                when(lower(col("full_text")).contains("débutant") | 
                     lower(col("full_text")).contains("introduction") |
                     lower(col("full_text")).contains("basics"), "Débutant")
                .when(lower(col("full_text")).contains("avancé") |
                      lower(col("full_text")).contains("expert") |
                      lower(col("full_text")).contains("advanced"), "Avancé")
    ...
    def _compute_statistics(self, df):
        ...
        try:
            ...
            stats = {
                ...
                "category_distribution": {row["predicted_category"]: row["count"] for row in category_stats},
                "level_distribution": {row["predicted_level"]: row["count"] for row in level_stats},
                "average_confidences": {
        ...
        try:
            ...
            processed_stream = stream_df \
                .withColumn("full_text", 
                    concat_ws(" ", col("titre"), col("description"))) \
