...
def export_to_csv(modeladmin, request, queryset):
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="{modeladmin.model._meta.model_name}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv"'
    )
...
def mark_as_completed(modeladmin, request, queryset):
    ...
    modeladmin.message_user(
        request, f"{updated} inscription(s) marquée(s) comme terminée(s)."
    )
...
class LearningPathAdmin(admin.ModelAdmin):
    ...
    def get_interests_display(self, obj):
        if obj.interests:
            interests = (
                ", ".join(obj.interests)
                if isinstance(obj.interests, list)
                else str(obj.interests)
            )
            return format_html(
                '<span title="{}">{}</span>',
                interests,
                interests[:50] + "..." if len(interests) > 50 else interests,
            )
        return "Aucun"
    ...
    def get_modules_display(self, obj):
        if obj.modules:
            return format_html(
                ...
                (
                    str(obj.modules)[:500] + "..."
                    if len(str(obj.modules)) > 500
                    else str(obj.modules)
                ),
            )
        return "Aucun module"
...
class QuizAdmin(admin.ModelAdmin):
    ...
    def get_attempts_count(self, obj):
        ...
        return format_html(
            "<div>{} tentatives<br/><small>{} réussies</small></div>", total, passed
        )
...
class QuizAttemptAdmin(admin.ModelAdmin):
    ...
    def get_duration(self, obj):
        if obj.completed_at and obj.started_at:
            ...
            return format_html(
                '<span title="{}">{}</span>', duration, str(duration).split(".")[0]
            )
        return "En cours..."
        ...
        try:
            ...
            stats = {
                ...
                "recent_count": (
                    qs.filter(
                        **{
                            f"{self.model._meta.get_field('created_at').name if hasattr(self.model, 'created_at') else 'id'}__gte": datetime.now()
                            - timedelta(days=7)
                        }
                    ).count()
