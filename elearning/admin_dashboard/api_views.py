...
class CourseImportAPIView(View):
    ...
    def post(self, request):
        ...
        try:
            ...
            update_existing = request.POST.get('update_existing', 'true').lower() == 'true'
            ...
            logger.info(f"Import API: {file.name} - {result.get('imported_count', 0)} cours importés")
...
def consumer_health_check(request):
    ...
    try:
        ...
        try:
            ...
            consumer_status = response.json() if response.status_code == 200 else {'status': 'error'}
        except:
