...
class DistributedRecommendationEngine:
    ...
    def _init_spark_session(self):
        ...
        try:
            self.spark = SparkSession.builder \
                .appName("InLearning-Recommendations") \
    ...
    def create_user_course_interactions(self, courses_df, users_df):
        ...
        try:
            ...
            interactions = self.spark.range(0, users_count * 20) \
    ...
    def generate_user_recommendations(self, user_id, num_recommendations=10):
        ...
        try:
            ...
            if results:
                ...
                logger.info(f"✅ {len(formatted_recs)} recommandations générées pour user {user_id}")
    ...
    def find_similar_courses(self, course_id, num_similar=5):
        ...
        try:
            ...
            if results:
                ...
                formatted_similar = [
                    {
                        "course_id": rec["user_id"],  # Dans item-item, user_id devient l'item similaire
                        "similarity_score": float(rec["rating"])
                    }
                    for rec in similar_courses
                ]
                
                logger.info(f"✅ {len(formatted_similar)} cours similaires trouvés pour course {course_id}")
    ...
    def compute_recommendation_metrics(self, interactions_df):
        ...
        try:
            ...
            active_users = interactions_df.groupBy("user_id").count() \
                .filter(col("count") >= 5).count()
            ...
            metrics = {
                ...
                "rating_distribution": {row["rating"]: row["count"] for row in rating_distribution}
            }
    ...
    try:
        ...
        if interactions:
            ...
            if training_results and training_results["model_trained"]:
                ...
                user_recs = engine.generate_user_recommendations(user_id=1, num_recommendations=5)
