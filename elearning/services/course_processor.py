...
class CourseProcessor:
    ...
    def process_course(self, course_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        processed_data["description"] = self._normalize_description(
            processed_data.get("description", "")
        )

        # Normaliser la difficulté
        processed_data["difficulty"] = self._normalize_difficulty(
            processed_data.get("difficulty", "beginner")
        )

        # Normaliser le mode d'apprentissage
        processed_data["learning_mode"] = self._normalize_learning_mode(
            processed_data.get("learning_mode", "text")
        )

        # Normaliser l'instructeur
        processed_data["instructor"] = self._normalize_instructor(
            processed_data.get("instructor", "")
        )

        # Valider et normaliser la durée
        processed_data["duration"] = self._normalize_duration(
            processed_data.get("duration", 1)
        )
    ...
    def _process_lessons(self, lessons: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for i, lesson in enumerate(lessons):
            processed_lesson = {
                "title": self._normalize_lesson_title(
                    lesson.get("title", f"Leçon {i+1}")
                ),
        ...
        for tag in tags:
            if isinstance(tag, str) and tag.strip():
                ...
                normalized_tag = re.sub(
                    r"[^\w\-]", "", normalized_tag
                )  # Garder seulement alphanumériques et tirets
    ...
    def validate_course_data(self, course_data: Dict[str, Any]) -> Dict[str, List[str]]:
        ...
        if difficulty and difficulty not in ["beginner", "intermediate", "advanced"]:
            errors.append(
                "La difficulté doit être 'beginner', 'intermediate' ou 'advanced'"
            )

        learning_mode = course_data.get("learning_mode")
        if learning_mode and learning_mode not in ["video", "text", "practice"]:
            errors.append(
                "Le mode d'apprentissage doit être 'video', 'text' ou 'practice'"
            )
