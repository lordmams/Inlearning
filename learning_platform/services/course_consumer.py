...
class CourseConsumer:
    ...
    def _create_directories(self):
        ...
        for directory in [self.watch_dir, self.processing_dir, self.processed_dir, self.error_dir]:
    ...
    def _validate_file(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            for i in range(int(max_wait / wait_interval)):
                ...
                logger.debug(f"Attente stabilité fichier {file_path.name}: {current_size} bytes")
    ...
    def _move_to_processing(self, file_path: Path) -> Path:
        ...
        if processing_path.exists():
            ...
            processing_path = self.processing_dir / f"{name_parts[0]}_{name_parts[1]}{name_parts[2]}"
    ...
    def _move_to_processed(self, file_path: Path) -> Path:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        name_parts = file_path.stem, timestamp, file_path.suffix
        processed_path = self.processed_dir / f"{name_parts[0]}_{name_parts[1]}{name_parts[2]}"
    ...
    def _send_to_learning_platform_api(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            learning_platform_url = os.getenv('LEARNING_PLATFORM_URL', 'http://flask_api:5000')
            ...
            if len(courses_data) == 1:
                ...
                logger.info(f"Envoi de {len(courses_data)} cours en {total_batches} lots au Learning Platform")
                
                for i in range(0, len(courses_data), batch_size):
                    batch = courses_data[i:i+batch_size]
                    batch_num = i//batch_size + 1
                    
                    logger.info(f"Traitement lot {batch_num}/{total_batches} ({len(batch)} cours)")
                    ...
                    logger.info(f"✅ Lot {batch_num}/{total_batches} traité avec succès")
                
                # Unifier les résultats
                result = all_results
            
            logger.info(f"Traitement terminé: {len(all_results) if isinstance(result, list) else 1} cours traités")
    ...
    def _transform_course_format(self, course_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'cours' in course_data:
                ...
                titre_str = str(titre_raw) if not isinstance(titre_raw, list) else ' '.join(str(x) for x in titre_raw)
                description_str = str(description_raw) if not isinstance(description_raw, list) else ' '.join(str(x) for x in description_raw)
    ...
    def _parse_file_data(self, file_path: Path, format_type: str) -> List[Dict[str, Any]]:
            ...
            if format_type == 'json':
                with open(file_path, 'r', encoding='utf-8') as file:
                    ...
                    if isinstance(data, list):
                        courses_data = [self._transform_course_format(course) for course in data]
                ...
                try:
                    ...
                    for _, row in df.iterrows():
                        ...
                        if course_data:
                            courses_data.append(self._transform_course_format(course_data))
        ...
        try:
            ...
            course_data = {
                ...
                "cours": {
                    ...
                    "contenus": {
                        "paragraphs": self._parse_list_field(row_data.get('paragraphs', '')),
                        "lists": self._parse_nested_list_field(row_data.get('lists', '')),
                        "examples": self._parse_list_field(row_data.get('examples', '')),
                        "texte": row_data.get('texte', row_data.get('main_text', '')),
                        "lienVideo": row_data.get('video_link', '')
                    },
                    "categories": self._parse_list_field(row_data.get('categories', '')),
    ...
    def _parse_list_field(self, field_value: str) -> List[str]:
        ...
        if not field_value or (hasattr(field_value, '__iter__') and str(field_value) == 'nan'):
            return []
        ...
        try:
            ...
            if field_value.startswith('['):
                ...
                for group in str(field_value).split(';'):
                    inner_list = [item.strip() for item in group.split(',') if item.strip()]
                    if inner_list:
