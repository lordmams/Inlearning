...
def classer_cours_avance(df):
    ...
    def extraire_mots_cles(text, min_length=3):
        ...
        return [
            word
            for word in tokens
            if word.isalpha() and len(word) >= min_length and word not in stop_words
        ]
...
def clustering_cours(df, n_clusters=5):
    ...
    for i in range(n_clusters):
        ...
        if cluster_indices:
            ...
            if all_keywords:
                ...
                cluster_names[i] = (
                    " & ".join(top_keywords) if top_keywords else f"Cluster {i+1}"
                )
            else:
    ...
    tsne = TSNE(
        n_components=2,
        random_state=42,
        perplexity=min(30, len(df) - 1) if len(df) > 1 else 1,
    )
...
def recherche_semantique(df, embeddings, model, requete, n_results=10, min_score=0.3):
    ...
    resultats = (
        df_temp[df_temp["similarite"] >= min_score]
...
def analyse_tendances(df):
    ...
    cours_par_mois = (
        df.groupby(["annee_mois", "category"]).size().reset_index(name="count")
    )
...
def main():
    st.markdown(
        '<h1 class="main-header">Système Avancé de Classification et d\'Analyse des Cours</h1>',
        unsafe_allow_html=True,
    )
    ...
    if json_file:
        ...
        with tab1:
            st.markdown(
                '<h2 class="sub-header">Visualisation interactive des cours</h2>',
                unsafe_allow_html=True,
            )
            ...
            with col1:
                ...
                with col_cat:
                    selected_categories = st.multiselect(
                        "Catégories", df["category"].unique().tolist(), default=[]
                    )

                with col_niv:
                    selected_levels = st.multiselect(
                        "Niveaux", df["level"].unique().tolist(), default=[]
                    )
                ...
                selected_clusters = st.multiselect(
                    "Clusters", df["cluster_name"].unique().tolist(), default=[]
                )
                ...
                if selected_levels:
                    df_filtered = df_filtered[
                        df_filtered["level"].isin(selected_levels)
                    ]
                if selected_sous_cat:
                    df_filtered = df_filtered[
                        df_filtered["sous_categorie"].isin(selected_sous_cat)
                    ]
                if selected_clusters:
                    df_filtered = df_filtered[
                        df_filtered["cluster_name"].isin(selected_clusters)
                    ]
            ...
            with col2:
                ...
                with metric_col1:
                    ...
                    st.markdown(
                        '<div class="metric-label">Cours sélectionnés</div>',
                        unsafe_allow_html=True,
                    )

                with metric_col2:
                    st.markdown(
                        f'<div class="metric-value">{df_filtered["category"].nunique()}</div>',
                        unsafe_allow_html=True,
                    )
                    st.markdown(
                ...
                with metric_col3:
                    if "nombre_mots" in df_filtered.columns:
                        ...
                        st.markdown(
                            f'<div class="metric-value">{avg_words}</div>',
                            unsafe_allow_html=True,
                        )
                        st.markdown(
                            '<div class="metric-label">Mots en moyenne</div>',
                            unsafe_allow_html=True,
                        )
                    else:
                        st.markdown(
            ...
            if viz_type == "Par cluster":
                st.plotly_chart(
                    visualiser_clusters(df_filtered), use_container_width=True
                )
            else:
                st.plotly_chart(
                    visualiser_categories(df_filtered), use_container_width=True
                )

        # Onglet 2: Recherche sémantique
        with tab2:
            st.markdown(
                '<h2 class="sub-header">Recherche sémantique avancée</h2>',
                unsafe_allow_html=True,
            )
            ...
            with col2:
                n_results = st.slider(
                    "Nombre de résultats", min_value=1, max_value=20, value=5
                )
                min_score = st.slider(
                    "Score minimum", min_value=0.0, max_value=1.0, value=0.3, step=0.05
                )
            ...
            with st.expander("Filtres de recherche avancés"):
                ...
                with col_niv:
                    search_levels = st.multiselect(
                        "Filtrer par niveau", df["level"].unique().tolist(), default=[]
                    )
            ...
            if requete:
                with st.spinner("Recherche en cours..."):
                    ...
                    if search_categories:
                        df_search = df_search[
                            df_search["category"].isin(search_categories)
                        ]
                    if search_levels:
                    ...
                    if len(df_search) != len(df):
                        ...
                        resultats = recherche_semantique(
                            df, embeddings, model, requete, n_results, min_score
                        )

                if len(resultats) > 0:
                    st.markdown(
                        '<h3 class="sub-header">Résultats de la recherche</h3>',
                        unsafe_allow_html=True,
                    )
                    ...
                    st.markdown(
                        f"<div class='highlight'>Score moyen de pertinence: {avg_score:.2f}</div>",
                        unsafe_allow_html=True,
                    )

                    # Trier les résultats similaires par catégorie
                    categories_similaires = (
                        resultats["category"].value_counts().reset_index()
                    )
                    ...
                    if len(categories_similaires) > 1:
                        ...
                        with col2:
                            ...
                            level_counts = (
                                resultats["level"].value_counts().reset_index()
                            )
                    ...
                    for i, (_, row) in enumerate(resultats.iterrows()):
                        with st.container():
                            ...
                            with col2:
                                st.markdown(
                                    f"<div class='highlight'>Score: {row['similarite']:.2f}</div>",
                                    unsafe_allow_html=True,
                                )
                            ...
                            **Niveau:** {row.get('level', 'Non spécifié')} | **Cluster:** {row.get('cluster_name', 'Non classifié')}
                            ...
                            if url:
                                st.markdown(
                                    f"[Accéder au cours]({url})", unsafe_allow_html=True
                                )
                    ...
                    st.warning(
                        "Aucun résultat ne correspond à votre recherche. Essayez de modifier votre requête ou de réduire le score minimum."
                    )

        # Onglet 3: Statistiques et Analyse
        with tab3:
            st.markdown(
                '<h2 class="sub-header">Statistiques et Analyse approfondie</h2>',
                unsafe_allow_html=True,
            )
            ...
            with col2:
                st.plotly_chart(
                    analyser_correlation_niveau_longueur(df), use_container_width=True
                )
            ...
            if "texte_complet" in df.columns:
                df["complexite_lexicale"] = df["texte_complet"].apply(
                    lambda x: (
                        len(set(word_tokenize(str(x).lower())))
                        / len(word_tokenize(str(x).lower()))
                        if len(word_tokenize(str(x).lower())) > 0
                        else 0
                    )
                )
                ...
                fig_complexite = px.box(
                    ...
                    color="level",
                    labels={
                        "complexite_lexicale": "Complexité lexicale",
                        "category": "Catégorie",
                    },
            ...
            fig_heatmap.update_layout(
                title="Corrélation entre catégories et sous-catégories"
            )
        ...
        with tab4:
            st.markdown(
                '<h2 class="sub-header">Analyse du vocabulaire</h2>',
                unsafe_allow_html=True,
            )
        ...
        with tab5:
            st.markdown(
                '<h2 class="sub-header">Explorer les données brutes</h2>',
                unsafe_allow_html=True,
            )
            ...
            if search_text:
                df_view = df_view[
                    df_view["title"].str.contains(search_text, case=False, na=False)
                ]

            # Afficher les données
            st.dataframe(
                df_view[colonnes_affichage], hide_index=True, use_container_width=True
            )
            ...
            if st.button("Exporter les données filtrées"):
                ...
                export_cols = [
                    ...
                    if col not in ["texte_complet", "mots_cles"]
                ]
