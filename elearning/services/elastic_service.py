    ...
    def index_course_json(self, course_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if response.get('result') in ['created', 'updated']:
                logger.info(f"✅ Cours '{cours.get('titre', 'Sans titre')}' indexé avec succès (ID: {course_id})")
                return True
            else:
                logger.error(f"❌ Échec de l'indexation du cours {course_id}: {response}")
                return False
            ...
            response = self.client.search(
                ...
                body={
                    ...
                    'aggs': {
                        ...
                        'learning_modes': {
                            'terms': {'field': 'learning_mode'}
                        },
                        'price_stats': {
                            'stats': {'field': 'price'}
                        },
                        'enrollment_stats': {
                            'stats': {'field': 'enrollment_count'}
                        }
                    }
                }
            )
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'index_exists': True,
                'document_count': index_stats['indices'][self.index_name]['total']['docs']['count']
            }
