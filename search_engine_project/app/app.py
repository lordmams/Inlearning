...
@app.route("/")
def home():
    return "🚀 Service Web Fonctionnel !"
...
def initialize_db():
    ...
    # 🔥 CRÉER UNE COPIE PROPRE DES DOCUMENTS SANS `_id`
    clean_documents = [{key: value for key, value in doc.items() if key != "_id"} for doc in documents]
...
def initialize_db():
    ...
    # 🔥 Supprimer le champ `_id` de chaque document pour éviter les doublons
    for doc in documents:
        doc.pop("_id", None)  # Supprime `_id` s'il existe
