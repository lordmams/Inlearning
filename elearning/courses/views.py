...
class CourseListView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        for hit in es_results.get('hits', {}).get('hits', []):
            ...
            category_colors = {
                'Programmation Générale': '#007bff', 'Science des Données': '#28a745',
                'Intelligence Artificielle': '#dc3545', 'Développement Web': '#fd7e14',
                'Base de Données': '#6f42c1', 'Sécurité': '#e83e8c'
            }
            ...
            course = {
                ...
                'content_length': len(course_data.get('cours', {}).get('contenus', {}).get('paragraphs', [])),
                'technologies': course_data.get('cours', {}).get('categories', [])[:3],  # Top 3 technologies
        ...
        stats = {
            ...
            'avg_difficulty': sum(c['difficulty_level'] for c in courses) / len(courses) if courses else 0,
        }

        # Options de tri
        sort_options = [
            {'value': 'relevance', 'label': 'Pertinence', 'icon': 'fas fa-sort-amount-down'},
        ...
        context = {
            ...
            'categories': [{'id': cat['name'], 'name': cat['name'], 'count': cat['enrollment_count']} for cat in categories_stats],
    ...
    def _estimate_reading_time(self, course_data):
        ...
        try:
            paragraphs = course_data.get('cours', {}).get('contenus', {}).get('paragraphs', [])
            total_words = 0
    ...
    def get(self, request, pk):
        ...
        if not es_result or not es_result.get('found'):
            ...
            try:
                ...
                course = {
                    ...
                    'category_name': django_course.category.name if django_course.category else 'Général',
            ...
            course = {
                ...
                'description': course_data.get('cours', {}).get('description', ''),
                'level': course_data.get('predictions', {}).get('predicted_level', 'Débutant'),
                'category_name': course_data.get('predictions', {}).get('predicted_category', 'Général'),
                'duration': course_data.get('cours', {}).get('duree', 'Non spécifié'),
                'url': course_data.get('cours', {}).get('lien', '#'),
                'instructor': 'Instructeur IA',
                'is_free': True,
                'content': course_data.get('cours', {}).get('contenus', {}).get('paragraphs', []),
        ...
        if request.user.is_authenticated:
            if pk.isdigit():
                ...
                try:
                    django_course = Course.objects.get(source_url=f"elasticsearch://{pk}")
                    is_enrolled = Enrollment.objects.filter(
                        user=request.user,
...
class EnrollCourseView(LoginRequiredMixin, View):
    def post(self, request, pk):
        try:
            ...
            messages.success(request, f"Vous êtes maintenant inscrit au cours '{course_title}'!")
            return redirect('course_detail', pk=pk)
