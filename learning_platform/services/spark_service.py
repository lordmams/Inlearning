...
class SparkService:
    ...
    def submit_course_processing_job(self, courses_data: List[Dict], job_id: str = None) -> Dict[str, Any]:
    ...
    def get_job_status(self, job_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response = requests.get(f"{self.spark_ui_url}/api/v1/applications", timeout=5)
            if response.status_code == 200:
                ...
                for app in applications:
                    if job_id in app.get("name", ""):
                        return {
                            "job_id": job_id,
                            "status": "running" if app.get("attempts", [{}])[-1].get("completed", True) == False else "completed",
                            "application_id": app.get("id"),
                            "start_time": app.get("attempts", [{}])[-1].get("startTime"),
                            "duration": app.get("attempts", [{}])[-1].get("duration", 0)
                        }
