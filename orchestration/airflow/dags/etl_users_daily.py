...
def load_users_to_postgres(**context):
    ...
    try:
        ...
        postgres_hook.insert_rows(
            table='users_person',
            rows=insert_data,
            target_fields=['name', 'age', 'gender', 'email', 'phone', 'predicted_level', 'created_at', 'updated_at'],
            replace=False
        )
    ...
    try:
        ...
        report = {
            ...
            'validation': {
                ...
                'data_quality_score': round((transform_info['rows_transformed'] / extract_info['rows_extracted']) * 100, 2)
            }
        }
        ...
        if report['validation']['data_quality_score'] < 80:
            logging.warning(f"Qualité des données faible: {report['validation']['data_quality_score']}%")
