...
class CourseImporter:
    ...
    def import_from_file(self, file, format_type: str, update_existing: bool = False, dry_run: bool = True) -> Dict[str, Any]:
        """
    ...
    def _validate_courses_data(self, courses_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for i, course_data in enumerate(courses_data):
            try:
                ...
                if course_data.get('difficulty') not in ['beginner', 'intermediate', 'advanced']:
                    ...
                    self.warnings.append(f"Cours {i+1}: Difficulté invalide, définie à 'beginner'")
                
                # Validation du mode d'apprentissage
                if course_data.get('learning_mode') not in ['video', 'text', 'practice']:
                    course_data['learning_mode'] = 'text'
                    self.warnings.append(f"Cours {i+1}: Mode d'apprentissage invalide, défini à 'text'")
    ...
    def _import_courses(self, courses_data: List[Dict[str, Any]], update_existing: bool):
        ...
        with transaction.atomic():
            for course_data in courses_data:
                try:
                    ...
                    logger.error(f"Erreur import cours '{course_data.get('title', 'Unknown')}': {e}")
                    self.errors.append(f"Erreur import '{course_data.get('title', 'Unknown')}': {e}")
        ...
        if existing_course:
            if update_existing:
                ...
                for field, value in course_data.items():
                    if field not in ['lessons', 'category_obj'] and hasattr(existing_course, field):
    ...
    def _index_course_in_elastic(self, course: Course):
        ...
        try:
            ...
            logger.warning(f"Erreur indexation Elasticsearch pour le cours {course.title}: {e}")
    ...
    def _generate_basic_template(self, format_type: str) -> str:
        ...
        if format_type == 'csv':
            return "title,description,category,instructor,duration,difficulty,learning_mode,price,is_free\n" + \
    ...
    def _generate_advanced_template(self, format_type: str) -> str:
        ...
        return "title,description,category,instructor,duration,difficulty,learning_mode,price,is_free,lessons,tags\n" + \
    ...
    def _generate_bulk_template(self, format_type: str) -> str:
        ...
        if format_type == 'csv':
            ...
            examples = [
                ("HTML & CSS Basics", "Apprendre les bases du HTML et CSS", "Web Development", "Alice Johnson", 8, "beginner", "text", 0, True, "Introduction HTML;Bases CSS;Responsive Design", "html,css,responsive"),
                ("JavaScript Moderne", "ES6+ et frameworks modernes", "Web Development", "Bob Smith", 12, "intermediate", "practice", 79.99, False, "ES6 Features;Async/Await;Modules", "javascript,es6,async"),
                ("React Fundamentals", "Créer des applications avec React", "Web Development", "Carol Brown", 16, "intermediate", "video", 129.99, False, "Components;State Management;Hooks", "react,components,hooks"),
                ("Node.js Backend", "Développement backend avec Node.js", "Backend", "David Wilson", 14, "advanced", "practice", 99.99, False, "Express Setup;Database Integration;API Design", "nodejs,express,api"),
                ("Database Design", "Conception de bases de données", "Database", "Eva Davis", 10, "intermediate", "text", 59.99, False, "SQL Basics;Normalization;Indexes", "sql,database,design")
            ]
            
            for title, desc, cat, instructor, duration, difficulty, mode, price, is_free, lessons, tags in examples:
                rows.append(f'"{title}","{desc}","{cat}","{instructor}",{duration},{difficulty},{mode},{price},{str(is_free).lower()},"{lessons}","{tags}"')
            ...
            examples = [
                ...
                ("React Fundamentals", "Web Development", "Carol Brown", "intermediate"),
