...
def test_dag_import(dag_path):
    ...
    try:
        ...
        dag_objects = [getattr(dag_module, attr) for attr in dir(dag_module) 
                      if attr == 'dag' or (hasattr(getattr(dag_module, attr), 'dag_id') if hasattr(dag_module, attr) else False)]
        
        if dag_objects:
            logger.info(f"✅ DAG importé avec succès: {dag_path}")
            return dag_objects[0] if hasattr(dag_objects[0], 'dag_id') else dag_objects[0]
        else:
    ...
    for task_id, task in dag.task_dict.items():
        ...
        if hasattr(task, 'python_callable'):
            try:
                ...
                issues.append(f"❌ Erreur lors de la validation de python_callable pour: {task_id}")
...
def main():
    ...
    dag_files = [f for f in os.listdir(dags_path) if f.endswith('.py') and not f.startswith('__')]
