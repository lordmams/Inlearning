...
class AnalyseSimpleDonneesEtudiants:
    ...
    def analyse_experience_professionnelle(self):
        ...
        print(f"Expérience minimum : {self.df_clean['total_experience_years'].min():.0f} ans")
        print(f"Expérience maximum : {self.df_clean['total_experience_years'].max():.0f} ans")
    ...
    def analyse_segments(self):
        ...
        adultes = self.df_clean[(self.df_clean['age'] > 25) & (self.df_clean['age'] <= 35)]
        seniors = self.df_clean[self.df_clean['age'] > 35]
        
        print(f"Segment Jeunes (18-25 ans) : {len(jeunes)} personnes ({len(jeunes)/len(self.df_clean)*100:.1f}%)")
