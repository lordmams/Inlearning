...
class AnalyseDonneesEtudiants:
    ...
    def nettoyer_donnees(self):
        ...
        df['total_experience_years'] = pd.to_numeric(df['total_experience_years'], errors='coerce')
    ...
    def analyse_demographique(self):
        ...
        self.df_clean['age_group'] = pd.cut(self.df_clean['age'], bins=age_bins, labels=age_labels, right=False)
    ...
    def analyse_langages_preferes(self):
        ...
        # Top 10 des langages préférés
        lang_dist = self.df_clean['preferred_language'].value_counts().head(10)
        print("Top 10 des langages de programmation préférés :")
        ...
        plt.title('Top 10 des Langages de Programmation Préférés', fontsize=14, fontweight='bold')
        plt.xlabel('Langage de Programmation')
        plt.ylabel('Nombre d\'étudiants')
        plt.xticks(rotation=45, ha='right')
        plt.tight_layout()
        plt.savefig('langages_preferes.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def analyse_modes_apprentissage(self):
        ...
        plt.pie(mode_dist.values, labels=mode_dist.index, autopct='%1.1f%%', startangle=90)
        plt.title('Distribution des Modes d\'Apprentissage', fontsize=14, fontweight='bold')
        plt.axis('equal')
        plt.tight_layout()
        plt.savefig('modes_apprentissage.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def analyse_niveaux_academiques(self):
        ...
        # Visualisation
        plt.figure(figsize=(12, 6))
        level_dist.plot(kind='bar', color='lightgreen')
        plt.title('Distribution des Niveaux Académiques', fontsize=14, fontweight='bold')
        plt.xlabel('Niveau Académique')
        plt.ylabel('Nombre d\'étudiants')
        plt.xticks(rotation=45, ha='right')
        plt.tight_layout()
        plt.savefig('niveaux_academiques.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def analyse_experience_professionnelle(self):
        ...
        print(f"Expérience moyenne : {self.df_clean['total_experience_years'].mean():.1f} ans")
        print(f"Expérience médiane : {self.df_clean['total_experience_years'].median():.1f} ans")
        print(f"Écart-type de l'expérience : {self.df_clean['total_experience_years'].std():.1f} ans")
        
        # Distribution par tranche d'expérience
        exp_bins = [0, 1, 3, 5, 10, 15, 30]
        exp_labels = ['0-1 an', '1-3 ans', '3-5 ans', '5-10 ans', '10-15 ans', '15+ ans']
        self.df_clean['exp_group'] = pd.cut(self.df_clean['total_experience_years'], 
                                           bins=exp_bins, labels=exp_labels, right=False)
        ...
        # Visualisation
        plt.figure(figsize=(12, 6))
        exp_dist.plot(kind='bar', color='orange')
        plt.title('Distribution de l\'Expérience Professionnelle', fontsize=14, fontweight='bold')
        plt.xlabel('Tranche d\'Expérience')
        plt.ylabel('Nombre d\'étudiants')
        plt.xticks(rotation=45, ha='right')
        plt.tight_layout()
        plt.savefig('experience_professionnelle.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def analyse_correlations(self):
        ...
        plt.scatter(self.df_clean['age'], self.df_clean['total_experience_years'], 
                   alpha=0.6, color='purple')
        plt.xlabel('Âge')
        plt.ylabel('Années d\'expérience')
        plt.title('Corrélation Âge vs Expérience Professionnelle', fontsize=14, fontweight='bold')
        
        # Ligne de régression
        z = np.polyfit(self.df_clean['age'], self.df_clean['total_experience_years'], 1)
        p = np.poly1d(z)
        plt.plot(self.df_clean['age'], p(self.df_clean['age']), "r--", alpha=0.8)
        
        plt.tight_layout()
        plt.savefig('correlation_age_experience.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def analyse_interets(self):
        ...
        top_interests = interest_counts.most_common(15)
        
        print("Top 15 des centres d'intérêt :")
        ...
        plt.barh(interests, counts, color='lightcoral')
        plt.title('Top 15 des Centres d\'Intérêt', fontsize=14, fontweight='bold')
        plt.xlabel('Nombre d\'occurrences')
        plt.gca().invert_yaxis()
        plt.tight_layout()
        plt.savefig('centres_interet.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def analyse_objectifs(self):
        ...
        # Top 10 objectifs court terme
        short_goals_counts = Counter(all_short_goals)
        top_short_goals = short_goals_counts.most_common(10)
        
        print("Top 10 des objectifs court terme :")
        for goal, count in top_short_goals:
            percentage = (count / len(all_short_goals)) * 100
            print(f"  {goal} : {count} ({percentage:.1f}%)")
        
        # Top 10 objectifs long terme
        long_goals_counts = Counter(all_long_goals)
        top_long_goals = long_goals_counts.most_common(10)
        
        print(f"\nTop 10 des objectifs long terme :")
    ...
    def analyse_complete(self):
        ...
        print("\n" + "=" * 80)
        print("ANALYSE TERMINÉE - Graphiques sauvegardés dans le répertoire courant")
        print("=" * 80)
...
if __name__ == "__main__":
    ...
    analyseur = AnalyseDonneesEtudiants(csv_file)
