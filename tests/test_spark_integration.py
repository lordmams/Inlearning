...
class TestSparkIntegration(unittest.TestCase):
    ...
    def test_spark_cluster_available(self):
        ...
        try:
            ...
            self.assertGreaterEqual(active_workers, 1, "Au moins 1 worker doit être actif")
    ...
    def test_api_spark_status(self):
        ...
        try:
            ...
            self.assertTrue(spark_cluster.get("enabled", False), "Spark devrait être activé")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if job_id:
                    ...
                    status_response = requests.get(f"{self.api_url}/spark/job-status/{job_id}")
                    
                    if status_response.status_code == 200:
                        status = status_response.json()
                        print(f"📈 Statut job: {status.get('status')}")
                        print(f"🆔 Application ID: {status.get('application_id', 'N/A')}")
                        
                        self.assertIn(status.get("status"), ["running", "completed", "submitted"])
                    else:
    ...
    def test_spark_performance_metrics(self):
        ...
        try:
            ...
            print(f"📊 Métriques Spark disponibles: {'Oui' if spark_metrics else 'Non'}")
            
            if spark_metrics and "error" not in spark_metrics:
                print(f"🏃 Applications totales: {spark_metrics.get('total_applications', 0)}")
                print(f"✅ Jobs complétés: {spark_metrics.get('completed_jobs', 0)}")
                print(f"⏱️ Temps total: {spark_metrics.get('total_processing_time', 0)}ms")
                print(f"📈 Utilisation cluster: {spark_metrics.get('cluster_utilization', 'unknown')}")
            else:
    ...
    def test_spark_ui_access(self):
        ...
        try:
            ...
            api_response = requests.get(f"{self.spark_ui_url}/api/v1/applications", timeout=5)
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if job_status in ["completed", "succeeded"]:
                        ...
                        print(f"❌ Job {job_id} échoué: {status.get('error', 'Unknown error')}")
                        return False
