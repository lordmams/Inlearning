...
class ElasticsearchService:
    def __init__(self):
        self.host = os.environ.get('ELASTICSEARCH_HOST', 'https://my-elasticsearch-project-d09d1e.es.us-central1.gcp.elastic.cloud')
    ...
    def search_courses(self, query: str = "*", size: int = 100, from_: int = 0, 
                      category: str = None, level: str = None, sort_by: str = "relevance") -> Dict[str, Any]:
        """
        ...
        try:
            ...
            response = requests.post(url, headers=self.headers, json=search_query, timeout=30)
    ...
    def get_all_courses(self, size: int = 1000) -> List[Dict[str, Any]]:
        ...
        try:
            search_query = {
                "size": size,
                "query": {"match_all": {}},
                "_source": ["cours.titre", "cours.description", "cours.niveau", 
                           "predictions.predicted_category", "predictions.predicted_level"]
            }
    ...
    def get_categories_stats(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            return [
                {
                    "name": bucket["key"],
                    "enrollment_count": bucket["doc_count"]
                }
        ...
        try:
            ...
            for item in data:
                try:
                    ...
                    response = requests.put(url, headers=self.headers, json=doc_data, timeout=10)
                    
                    if response.status_code in [200, 201]:
                        imported_count += 1
                    else:
                        errors.append(f"Erreur pour document {doc_id}: {response.status_code}")
