...
def convert_json_to_csv(json_file_path, csv_file_path):
    ...
    for entry in data:
        ...
        processed_data.append({
            ...
            "highest_academic_level": academic_background.get("highest_academic_level", ""),
            "fields_of_study": ", ".join([field.get("field_name", "") for field in academic_background.get("fields_of_study", [])]),
            "institution": ", ".join([field.get("institution", "") for field in academic_background.get("fields_of_study", [])]),
            "year_of_completion": ", ".join([str(field.get("year_of_completion", "")) for field in academic_background.get("fields_of_study", [])]),
            "total_experience_years": professional_background.get("total_experience_years", ""),
            "jobs": ", ".join([job.get("title", "") for job in professional_background.get("jobs", [])]),
...
json_file_path = "generated_student_data.json"  # Remplacez par le chemin de votre fichier JSON
