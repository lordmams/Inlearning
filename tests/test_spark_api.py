...
def test_single_course():
    ...
    course_data = {
        ...
        "contenus": {
            ...
            "lists": [["Variables", "Functions", "Classes"], ["if", "while", "for"]],
            "examples": [
                "print('Hello World')",
                "x = 5\nprint(x)",
                "def hello():\n    return 'Bonjour'",
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for i, result in enumerate(results):
                print(
                    f"   Cours {i+1}: {result.get('titre')} → {result.get('predicted_category')}"
                )
            return True
        else:
...
def test_stats():
    ...
    try:
        response = requests.get(
            "http://localhost:8000/stats/processed-courses", timeout=5
        )
        if response.status_code == 200:
            data = response.json()
...
def main():
    ...
    if passed == total:
        print(
            "🎉 Tous les tests sont passés! L'API Learning Platform fonctionne correctement."
        )
        ...
        print(
            "Vérifiez que l'API est démarrée avec: cd learning_platform/api && python app.py"
        )
        return 1
